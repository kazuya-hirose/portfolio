...
日本の投資家が時点0で米国株 A 株1株を購入したとしよう。購入時点の円建ての金額は $30\times100=3{,}000$ 円であった。1年後の A 株の円建ての価値は $36\times110=3{,}960$ 円であるから
$$
...
一方，米国の投資家が日本株 J 株1株を購入すると，**通貨リターンが与える影響が日本の投資家と逆**になる。購入時点のドル建ての購入代金は $500\div100=5$ ドル，1年後の J 株のドル建ての価値は $600\div110=5.45$ ドルである。したがって
$$
...
以上のことを一般的に説明してみる。外国資産の自国通貨建てリターン $R$ は
$$
R=\frac{P_1S_1}{P_0S_0}-1=(1+R^L)(1+s)-1=R^L+s+R^Ls\simeq R^L+s
$$

ここで $P_0$ は0時点の外国資産の現地通貨建て価格，$S_0$ は0時点の為替レート，$R^L$ は現地通貨建てリターン，$s$ は通貨リターンである。

すなわち，短期においては $R^Ls$ は無視できる大きさであるため，近似的に
$$
\text{自国通貨建てリターン}\simeq\text{現地通貨建てリターン}+\text{通貨リターン}
$$
が成り立つ。
...
$$
R\equiv\log\!\left(\frac{P_1S_1}{P_0S_0}\right)=\log\frac{P_1}{P_0}+\log\frac{S_1}{S_0}=R^L+s
$$
...
ヘッジ比率が高くなるほど為替リスクの影響が小さくなる。$h=0$ のときは**ヘッジなし**，$h=1$ のときは**完全ヘッジ**，$0<h<1$ は**部分ヘッジ**と呼ばれる。
...
# 6.4.8 国際CAPMの拡張——グローバル・マルチファクター・モデル
...
- 外国資産の自国通貨建てリターンは $R\simeq R^L+s$（現地通貨建てリターン＋通貨リターン）に分解できる。円安は日本の投資家にプラス，米国の投資家にマイナスの通貨リターンをもたらす。
- **ヘッジ比率** $h$ の部分ヘッジのリターンは $R^L+(1-h)s+hf$，すなわちヘッジなしと完全ヘッジの加重平均。完全ヘッジでは通貨リターンが消えて**フォワード・プレミアム率**が付く。
- 各国資産の相関は低いので国際分散投資で効率的フロンティアは**北西へ改善**する。ただしグローバル化・市場統合・危機時の相関上昇がその利益を縮小させる。

---

# まとめ（2）

- 為替レートの長期理論は**購買力平価**（絶対的・相対的 PPP），短期理論は**カバーなしの金利平価**。投資家が関心を持つのは PPP からの乖離＝**実質為替リスク**である。
- カバー付き金利平価は**フォワードレートの決定式**（無裁定条件）であり，直物レートの均衡条件ではない。カバーなし金利平価は $b=1$ が実証で棄却され，**フォワード・プレミアム・パズル**として未解決。
- **為替のリスクプレミアム** $SRP=r^{*}+E(s)-r$ はカバーなし金利平価からの乖離。どの通貨建てで見るかによって符号が変わる点が，通常の危険資産と異なる。

---

# まとめ（3）

- **国際CAPM**：第1定理は「自国リスクフリー資産＋為替リスクを最適にヘッジした世界マーケット・ポートフォリオ」の分離定理，第2定理は世界マーケット・ベータによる証券市場線。
- 国際 CAPM が成立すれば国内マーケット・ポートフォリオは効率的でなくなり，国内版の証券市場線も成立しない。実証では**世界マーケット・ベータも国内マーケット・ベータも説明力を持つ**（市場の分断）。
- **ホームバイアス**は国際分散投資の有名なパズル。規制・情報アクセス・国際CAPMの限界という仮説があるが，いずれも十分な説明には至っていない。
