...
第2章では，投資家が平均・分散アプローチに従って最適なポートフォリオを選ぶ姿を描いた。本章では，**すべての投資家がそのように行動した結果として市場価格がどう決まるか**を問う。CAPM（Capital Asset Pricing Model）は，市場の均衡状態を記述する2つの定理からなる。第1定理は「マーケット・ポートフォリオは効率的である」，第2定理は「証券のリスクプレミアムはベータに比例する」と主張する。
...
- **CAPM第1定理**：安全資産があるとき，トービンの分離定理により全投資家が接点ポートフォリオを選ぶ。需給一致から，均衡ではマーケット・ポートフォリオ＝接点ポートフォリオとなり，**効率的**である。安全資産がなくても（ゼロベータCAPM）成立する。
- **資本市場線**は $\mu_P=r_f+[(\mu_M-r_f)/\sigma_M]\sigma_P$。傾き＝シャープ比で，$M$ はシャープ比最大のポートフォリオ。
- **ベータ** $\beta_i=\sigma_{iM}/{\sigma_M}^2$ は，リターンをベクトルと見たとき $R_i$ の $R_M$ 方向成分の大きさ。ポートフォリオのベータは加重平均。
- **リスク分解** ${\sigma_i}^2={\beta_i}^2{\sigma_M}^2+{\sigma_{e_i}}^2$。市場関連リスクの割合は ${\rho_{iM}}^2$ に等しい。

---

# まとめ（2）

- **CAPM第2定理** $\mu_i-r_f=(\mu_M-r_f)\beta_i$。証券市場線の $x$ 軸はベータ（CMLの $x$ 軸は $\sigma$）。市場は**市場関連リスクにのみ対価を払い，非市場リスクには払わない**。
- **ベータの推定**はマーケット・モデルの回帰による。個別銘柄は推定誤差が大きく，ベータは1に平均回帰する傾向がある。
- **CAPMの利用**：インデックス運用，ジェンセンのアルファによるパフォーマンス評価，アクティブ運用の銘柄選択，資本コスト（WACC）。
- **実証**：日本株では第2定理は確認できるが有意性は弱く，**バリュー株効果が強い**。データ・スヌーピング，サバイバーシップ，非正規性，**ロールの批判**，多期間への拡張という5つの問題が残る。
